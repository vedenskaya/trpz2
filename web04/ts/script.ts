...
class Computer extends Product {
  haveColors: string[]; //Цвета которые есть
  haveProc: string[]; 
  constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public list?:Spec[]) {
    super(id, name, price, description, inStock);
    this.CalculateFlags();
    this.Init();
  }
...
function CheckRedComp(flag: boolean) {
  document.getElementById('rowts').innerHTML = "";
  if (flag) {
    for (let i = 0; i < this.productList.length; i++){
      if (productList[i] instanceof Computer)
        for (let j = 0; (<Computer>productList[i]).list.length; j++)
          if ((<Computer>productList[i]).list[j].color == Color.Red) 
            (<Computer>productList[i]).Init();
    }
  }
  else {
    for (let i = 0; i < this.productList.length; i++)
...
class Outerwear extends Product {
  ...
  constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public list?:Wear[]) {
    super(id, name, price, description, inStock);
    this.CalculateFlags();
    this.Init();
  }
